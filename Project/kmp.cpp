...
int findOverlap(const string& text, const string& pattern, int max_length, const vector<int>& lps) {
    int N = text.size();
    int M = pattern.size();
...
void kmpMerge(int pattern_length) {
    ...
    computeLPSArray(reads[0].substr(0, pattern_length), pattern_length, lps);
    ...
    while (!reads.empty()) { // 모든 read가 병합될 때까지 반복
        ...
        for (auto it = reads.begin(); it != reads.end();) {
            int overlap = findOverlap(result, *it, pattern_length, lps);
            ...
            overlap = findOverlap(*it, result, pattern_length, lps);
